...
open class MenuItemView: UIView {
    public lazy var titleLabel: UILabel = self.initLabel()
    public lazy var descriptionLabel: UILabel = self.initLabel()
    public lazy var menuImageView: UIImageView = {
        $0.isUserInteractionEnabled = true
        $0.translatesAutoresizingMaskIntoConstraints = false
        return $0
    }(UIImageView(frame: .zero))

    fileprivate(set) public var customView: UIView? {
        didSet {
    ...
    internal(set) public var isSelected: Bool = false {
        didSet {
    ...
    fileprivate(set) public lazy var dividerImageView: UIImageView? = { [unowned self] in
    ...
    internal init(menuOptions: MenuViewCustomizable, menuItemOptions: MenuItemViewCustomizable, addDiveder _: Bool) {
