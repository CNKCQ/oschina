...
open class PagingViewController: UIViewController {
    ...
    internal(set) public var currentViewController: UIViewController!
    fileprivate(set) public var visibleControllers = [UIViewController]()
        ...
        for (index, controller) in controllers.enumerated() {
            ...
            if !shouldLoad(page: index) {
                ...
                if isVisible(controller: controller) {
                    ...
                    _ = visibleControllers.index(of: controller).flatMap { visibleControllers.remove(at: $0) }
                }
                continue
            }
    ...
    fileprivate func layoutPagingViewControllers() {
        ...
        for (index, controller) in controllers.enumerated() {
            ...
            if options.lazyLoadingPage == LazyLoadingPage.one ||
                ...
                if case let .all(menuOptions, _) = options.componentType,
...
extension PagingViewController: Pagable {
    ...
    var previousPage: Int {
        guard case let .all(menuOptions, _) = options.componentType, case .infinite = menuOptions.displayMode else { return currentPage - 1 }
...
extension PagingViewController {
    ...
    fileprivate func isVisible(controller: UIViewController) -> Bool {
        return childViewControllers.contains(controller)
    }
