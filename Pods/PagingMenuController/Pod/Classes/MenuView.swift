...
open class MenuView: UIScrollView {
    fileprivate(set) public var currentMenuItemView: MenuItemView!

    fileprivate(set) internal var menuItemViews = [MenuItemView]()
    ...
    fileprivate lazy var roundRectView: UIView = {
    ...
    fileprivate func constructMenuItemViews(_ menuOptions: MenuViewCustomizable) {
        constructMenuItemViews({
            MenuItemView(menuOptions: menuOptions, menuItemOptions: menuOptions.itemsOptions[$0], addDiveder: $1)
        })
    }
