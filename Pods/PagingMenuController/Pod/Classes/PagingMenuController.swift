...
open class PagingMenuController: UIViewController {
    fileprivate(set) public var menuView: MenuView? {
        didSet {
    ...
    fileprivate(set) public var pagingViewController: PagingViewController? {
        didSet {
    ...
    open func setup(_ options: PagingMenuControllerCustomizable) {
        ...
        case let .all(menuOptions, _):
            self.menuOptions = menuOptions
        case let .menuView(menuOptions):
            self.menuOptions = menuOptions
        default: break
        }
    ...
    fileprivate func constructPagingViewController() {
        ...
        switch options.componentType {
        case let .pagingController(pagingControllers): viewControllers = pagingControllers
        case let .all(_, pagingControllers): viewControllers = pagingControllers
        default: return
        }
...
extension PagingMenuController: Pagable {
    public var currentPage: Int {
        ...
        case let (_, pagingViewController?):
...
extension PagingMenuController {
    ...
    fileprivate func validateContentsCount(_ options: PagingMenuControllerCustomizable) {
        switch options.componentType {
        case let .all(menuOptions, pagingControllers):
    ...
    fileprivate func validateDefaultPage(_ options: PagingMenuControllerCustomizable) {
        let maxCount: Int
        switch options.componentType {
        case let .pagingController(pagingControllers):
            maxCount = pagingControllers.count
        case let .all(_, pagingControllers):
            maxCount = pagingControllers.count
        case let .menuView(menuOptions):
    ...
    fileprivate func validateInfiniteMenuItemNumbers(_ options: PagingMenuControllerCustomizable) {
        guard case let .all(menuOptions, _) = options.componentType,
