...
//    private func getArticleInfoByPage(page: Int) -> Observable<[ArticleEntity]> {
        //        return Observable
//            .zip(
//                provider.request(GankIOService.byPageAndKind(kind: "福利", page: page, count: offset), completion: <#Completion#>).filter(statusCodes: 200 ... 201).observeOn(backgroundWorkScheduler).map({ response -> [ArticleEntity] in
